...
| ['turn1-template', configId] | Single Turn1Template record for the ScenarioConfig. Used by Step 5 as an **existence probe only** — `data != null` locks `dimension_key` re-derivation so display_name renames preserve the existing key. 404 from `getTurn1Template` is caught in the queryFn and resolved to `null` (same pattern as `['tension', …]`). Not currently invalidated by any frontend mutation — Turn1Template authoring is out of scope for the polish session; once the Turn1Template editor lands its create/update mutations should invalidate this key. | (none yet — see note). |
